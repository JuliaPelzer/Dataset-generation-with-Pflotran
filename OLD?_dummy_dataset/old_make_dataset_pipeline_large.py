...
def run_simulation(args):
    ...
    if args.num_hps > 1:
        assert (
            args.vary_hp
        ), f"If number of heatpumps is larger than 1, vary_hp must be True"
    ...
    # copy settings file
    shutil.copy(
        "dummy_dataset_pipeline_large/settings_2D_large.yaml",
        f"{output_dataset_dir}/inputs/settings.yaml",
    )
    if args.benchmark or (args.num_hps - args.vary_hp_amount > 0):
        shutil.copy(
            "dummy_dataset_pipeline_large/benchmark_locs_hps.yaml",
    ...
    write_hp_additional_files(".", args.num_hps, args.vary_hp_amount)

    # potentially calc 1 or 2 hp locations
    if args.vary_hp:
        locs_hps = calc_loc_hp_variation_2d(
            args.num_dp,
            f"{output_dataset_dir}/inputs",
            args.num_hps,
            settings,
            benchmark_bool=args.benchmark,
            num_hps_to_vary=args.vary_hp_amount,
        )
