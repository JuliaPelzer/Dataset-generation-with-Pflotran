...
from scripts.write_benchmark_parameters_input_files import \
    write_parameter_input_files


def run_simulation(args):
    logging.info(f"Working at {datetime.datetime.now()} on folder {os.getcwd()}")
    assert args.num_hps in [
        0,
        1,
        2,
    ], f"Number of heatpumps must be 0, 1 or 2 but it is {args.number_hps}"
    if args.num_hps > 1:
        assert (
            args.vary_hp
        ), f"If number of heatpumps is larger than 0, hp_variation must be True"
    assert (
        args.num_dp == 0 and args.vary_perm == False and args.vary_hp == False
    ), "If num_dp=0 -> testcases => vary_perm and vary_hp must be False"
    ...
    if args.dims == "2D":
        shutil.copy(
            "dummy_dataset_benchmark/settings_2D.yaml",
            f"{output_dataset_dir}/inputs/settings.yaml",
        )

    else:
        shutil.copy(
            "dummy_dataset_benchmark/settings_3D_fine.yaml",
            f"{output_dataset_dir}/inputs/settings.yaml",
        )
    ...
    if args.vary_hp:
        locs_hps = calc_loc_hp_variation_2d(
            args.num_dp, f"{output_dataset_dir}/inputs", args.num_hps, settings
        )
    ...
    for run_id in range(
        len(pressures)
    ):  # should only differ in case of benchmark_4_testcases from CLA_DATAPOINTS
        ...
        if not args.vary_hp:
            ...
            raise ValueError(
                "If hp variation is True, number of hps must be larger than 0"
            )

        logging.info(
            f"Starting PFLOTRAN simulation of RUN {run_id} at {datetime.datetime.now()}"
        )
        remote = True
        if not remote:
            os.system(
                f"mpirun -n 1 $PFLOTRAN_DIR/src/pflotran/pflotran -output_prefix {output_dataset_run_dir}/pflotran -screen_output off"
            )
        else:
            ...
            os.system(
                f"mpirun -n 32 {PFLOTRAN_DIR}/pflotran -output_prefix {output_dataset_run_dir}/pflotran -screen_output off"
            )
        ...
        if args.visu:
            plot_sim(
                output_dataset_run_dir, settings, case="2D", confined=confined_aquifer
            )
...
def just_visualize(args):
    ...
    for run_id in range(4):  # in case of testcases_4
        output_dataset_run_dir = f"{output_dataset_dir}/RUN_{run_id}"
        plot_sim(
            output_dataset_run_dir, settings, case=args.dims, confined=confined_aquifer
        )
        logging.info(f"...visualisation of RUN {run_id} is done")
...
if __name__ == "__main__":
    ...
    )  # int = 100 # number of datapoints; if 0: testcases (4)
    parser.add_argument(
        "--name", type=str, default="default"
    )  # benchmark_dataset_2d_100dp_vary_perm")
