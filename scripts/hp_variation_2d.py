...
def calc_hps_locs_float(vary_poss: bool, num_dps: int, num_hps: int, settings: Dict):
    ...
    if vary_poss:
        try:
            locs_x = np.random.uniform(0 + distance_to_border[0][0],grid_size[0] - distance_to_border[0][1],num_dps*num_hps,)
        except:
            try:
                locs_x = np.random.uniform(0 + distance_to_border[0][0],grid_size[0] - distance_to_border[0][0],num_dps*num_hps,)
            except:
                locs_x = np.random.uniform(0 + distance_to_border,grid_size[0] - distance_to_border,num_dps*num_hps,)

        if len(distance_to_border[1]) == 1:
            locs_y = np.random.uniform(0 + distance_to_border[1],grid_size[1] - distance_to_border[1],num_dps*num_hps,)
        elif len(distance_to_border[1]) == 2:
            locs_y = np.random.uniform(distance_to_border[1][0],grid_size[1] - distance_to_border[1][1],num_dps*num_hps,)
