...
def get_grid(
    ...
    # if ids.shape[2] == 1:
    #     place_centers = (~leave_out) & (ids[1:-1, 1:-1, 0] == -1)
    #     ids[1:-1, 1:-1, 0][place_centers] = np.arange(place_centers.sum()) + index_offset
    # else:
    place_centers = (~leave_out) & (ids[1:-1, 1:-1, 1:-1] == -1)
    ids[1:-1, 1:-1, 1:-1][place_centers] = np.arange(place_centers.sum()) + index_offset
    ...
    depth_face_centers = np.stack(  
        np.meshgrid(
            np.linspace(minx + cell_w / 2, maxx - cell_w / 2, res_x, endpoint=True),
            np.linspace(miny + cell_h / 2, maxy - cell_h / 2, res_y, endpoint=True),
            np.linspace(minz,              maxz,              res_z + 1, endpoint=True),
        )
    ).transpose(2,1,3,0)
    ...
    # if ids.shape[2] == 1: # TODO correct?
    #     left_valid = valid[:-1, 1:-1]
    #     right_valid = valid[1:, 1:-1]
    # else:
    left_valid = valid[:-1, 1:-1, 1:-1]
    right_valid = valid[1:, 1:-1, 1:-1]
    ...
    # else:
    up_valid = valid[1:-1:, 1:, 1:-1]
    down_valid = valid[1:-1, :-1, 1:-1]
    horizontal_add &= down_valid
    horizontal_add &= up_valid

    # if ids.shape[2] == 1: # TODO TODO correct?
    #     front_valid = valid[1:-1, 1:-1]
    #     back_valid = valid[1:-1, 1:-1]
    # else:
    ...
    # else:
    vertical_add &= ~(larger_grid[:-1, 1:-1, 1:-1] & larger_grid[1:, 1:-1, 1:-1])
    horizontal_add &= ~(larger_grid[1:-1, :-1, 1:-1] & larger_grid[1:-1, 1:, 1:-1])
    ...
    # else:
    vertical_diff_valid = np.diff(ids, axis=0)[:, 1:-1, 1:-1] != 0
    horizontal_diff_valid = np.diff(ids, axis=1)[1:-1, :, 1:-1] != 0
    depth_diff_valid = np.diff(ids, axis=2)[1:-1, 1:-1, :] != 0
    depth_add &= depth_diff_valid
    # TODO check if vertical_add vorher ist das selbe wie nach hinzufügen von vertical_:diff_valid
    ...
    # else:
    vertical_face_left_neighbor = ids[:-1, 1:-1, 1:-1][vertical_add]
    vertical_face_right_neighbor = ids[1:, 1:-1, 1:-1][vertical_add]
    horizontal_face_down_neighbor = ids[1:-1, :-1, 1:-1][horizontal_add]
    horizontal_face_up_neighbor = ids[1:-1, 1:, 1:-1][horizontal_add]
    depth_face_front_neighbor = ids[1:-1, 1:-1, :-1][depth_add] # TODO
    depth_face_back_neighbor = ids[1:-1, 1:-1, 1:][depth_add] # TODO
...
def double_size(ids):
    # TODO fix this for more than z = 1
    w, h, _ = ids.shape
    new_ids = -np.ones((w * 2, h * 2, 1), dtype=ids.dtype)
    new_ids[::2, ::2,:] = ids
    new_ids[1::2, ::2,:] = ids
    new_ids[::2, 1::2,:] = ids
    new_ids[1::2, 1::2,:] = ids
...
class Grid:
    ...
    def base_chunks(self):
        ...
        """
        assert self.res_z == 1, "3D chunks not implemented"
...
class Chunk:
    ...
    def valid_subdivisions(self):
        ...
        Even for (fake)3D, dimension z is not chunked
        """
        for x in range(2):
            for y in range(2):
    ...
    def indices_with_neighbors(self):
        ...
        """
        all_indices = -np.ones((self.res_x + 2, self.res_y + 2,self.res_z+2), dtype=int)
        all_indices[1:-1, 1:-1, 1:-1] = self.indices
        if self.idx > 0:
        ...
        self.indices = all_indices[1:-1, 1:-1,1:-1]
        return all_indices
    ...
    def plot(self, ax=None, **kwargs):
        if ax is None:
            ax = plt.gca()
        xmin, xmax, ymin, ymax, _, _ = self.extent
        ax.add_patch(
            plt.Rectangle(
                (xmin, ymin),
                xmax - xmin,
