...
def cut_bcs_hh(tok:np.array, gwgl:np.array, cells: Dict[str, np.ndarray], desti_resolution:int, box_length_in_m: float) -> Dict[str, np.ndarray]:
    ...
    for direction in ["north", "south", "west", "east"]:
        bc_locs[direction] = (cells[direction]//desti_resolution).astype(int)
        # print(bc_locs[direction][:10], gwgl.shape)

    tok_min = np.nanmin(tok)

    bcs = {}
    bcs["inflow"]   = - np.median(gwgl[bc_locs["west"]] - tok_min)/box_length_in_m
    bcs["outflow"]  = - np.median(gwgl[bc_locs["east"]] - tok_min)/box_length_in_m
    bcs["left"]     = - np.median(gwgl[bc_locs["south"]] - tok_min)/box_length_in_m  # in stream direction
    bcs["right"]    = - np.median(gwgl[bc_locs["north"]] - tok_min)/box_length_in_m  # in stream direction
