...
def estimate_box_size(properties, start, field_size = np.array([100,100]), safety_factor:float = 1.0, method: str = "TAP"):
    # Formel aus Poster von Fabian
    v_d = sample_median(properties["darcy_velocity"], start, field_size) # Darcy vel [m/s]
    b = sample_median(properties["thickness"], start, field_size) # Aquifer thickness [m]
    v_dd = sample_median(properties["drawdown"], start, field_size) # max-drawdown # TODO Einheitsumrechnung -> SI-Einheiten
...
def make_window_shape(window_shape, resolution, properties_full, start):
    """
    ...
    if window_shape is None:
        ...
        print("manually set window_shape", window_shape)
    window_shape = np.array([window_shape[0]/resolution, window_shape[1]/resolution]) # convert to cells
    window_shape = window_shape.astype(int)
    for i in range(2):
        if window_shape[i] == 0:
            window_shape[i] = 2
            print(f"WARNING: window_shape too small (in at least one direction zero) -> set to 2")
    print("size of simulation box in cells", window_shape)
    return window_shape

def estimate_box_rotation(direction_field, start, window_shape):
