...
def plot_sim(
    ...
    try:
        ...
        logging.info(
            f"Temperature at HP: {np.round(list_to_plot[11]['data'][23,9], 4)}"
        )
    ...
    for time in hdf5_file.keys():
        if not time in [
            ...
            for property in hdf5_file[time].keys():
                ...
                if reshape_bool:
                    data_dict["data"] = data_dict["data"] = data_dict["data"].reshape(
                        dimensions, order="F"
                    )
                if case == "side_hp":
...
def _plot_isolines(
    ...
    path: str,
    settings,
    name_pic: str = "plot_isolines_exemplary",
    case: str = "side_hp",
):
    # helper function to plot the data
    ...
    for data_point in data:
        if (
            data_point["property"] == "Temperature [C]"
            and data_point["time"] != "   1 Time  1.00000E-01 y"
        ):
            plt.sca(axes[index])
            plt.contourf(
                data_point["data"],
...
def _plot_temperature_field(
    data, path: str, settings, name_pic: str = "plot_temp_field_exemplary"
):
    """
    ...
    logging.info(f"Resulting picture is at {pic_file_name}")
    plt.suptitle(
        f"Temperature field and isolines of {T_gwf}, {T_gwf+1} and {T_gwf+T_inj_diff} °C"
    )
    plt.savefig(f"{pic_file_name}.png")
    plt.savefig(f"{pic_file_name}.svg")
...
def plot_perm(
    path_settings: str = "try_perm", path_run="try_perm/RUN_0", case="top_hp"
):
